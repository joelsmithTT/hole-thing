...
inline MappedMemory bh_map_noc2axi(int fd)
{
    ...
    auto resource = get_mapping(fd, TENSTORRENT_MAPPING_RESOURCE0_UC);
...
{
    auto resource = get_mapping(fd, TENSTORRENT_MAPPING_RESOURCE1_UC); // BAR2 is index 1
...
{
    auto resource = get_mapping(fd, TENSTORRENT_MAPPING_RESOURCE2_WC); // BAR4 is index 2 (BAR2 is index 1)
    void* mem = mmap(nullptr, resource.mapping_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, resource.mapping_base);
